def posicionesInt(posiciones):
    posicion_int = []
    for posicion in posiciones:
        posicion_int.append(int(posicion))
    return posicion_int

    
def calcularPuntos(posiciones_int):
    puntos = 0
...
def ListadoTotalPuntos(archivo):
    with open(archivo, 'r') as archivo:
        ...
        for linea in lineas[1:]:                  
            ...
            posiciones_int = posicionesInt(posiciones)
            puntos_piloto = calcularPuntos(posiciones_int)
...
def ListadoPuntosEquipos(archivo):
    with open(archivo, 'r') as archivo:
        ...
        for linea in lineas[1:]:
            ...
            posiciones = linea.split()[6:]
            posiciones_int = posicionesInt(posiciones)

            puntos_pilotos = calcularPuntos(posiciones_int)
...
def PromedioPuntos(archivo):
    with open(archivo, 'r') as archivo:
        ...
        for linea in lineas[1:]:
            ...
            for i in posiciones_int:
                carreras += 1
            promedio = puntos_piloto/carreras
            print(f"{pilotos}: {promedio:.2f} ")
...
def MejorPosicion(archivo):
    with open(archivo, 'r') as archivo:
        ...
        for i in range(n):
            for j in range(0, n - i - 1):
                if piloto[j] > piloto[j + 1]:
                    piloto[j], piloto[j + 1] = piloto[j + 1], piloto[j]
                    mejor_posicion[j], mejor_posicion[j + 1] = mejor_posicion[j + 1], mejor_posicion[j]
        for i in range(n):
...
while True:
    ...
    if opcion == '1':
        print("---------------------------------")
        print("Puntos totales de los pilotos: ")
        print("---------------------------------")
